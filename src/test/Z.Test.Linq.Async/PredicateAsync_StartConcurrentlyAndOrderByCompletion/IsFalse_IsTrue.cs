...
namespace Z.Test.Linq.Async
{
    ...
    {
        [TestMethod]
        public async Task IsFalse_IsTrue()
        {
            ...
            var result = await enumerable.WhereAsync(x => predicateAsync.Predicate(x)).StartPredicateConcurrently(false).OrderByPredicateCompletion(true).ToList();
            ...
            // MUST be ordered be completion for predicate (OrderByPredicateCompletion always start predicate concurrently)
