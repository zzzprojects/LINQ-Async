...
namespace Z.Test.Linq.Async
{
    ...
    {
        [TestMethod]
        public async Task IsTrue_IsFalse()
        {
            ...
            var result = await enumerable.WhereAsync(x => predicateAsync.Predicate(x)).StartPredicateConcurrently(true).OrderByPredicateCompletion(false).ToList();
